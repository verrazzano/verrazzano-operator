...
watches local custom resource definitionss for models/bindings, and launches micro operators
...
- On creation or update of a `VerrazzanoModel`/`VerrazzanoBinding` pair, the Verrazzano Operator launches:
  ...
  - Into the Managed Cluster for each "component":
    ...
    - Agents forwaring logs and metrics to the Verrazzanoe Monitoring Instance.
...
verrazzano-operator, verrazzano-monitoring-operator, and so on, into a single Helm chart.
...
See the [values.yaml](./chart/values.yaml) for the full list of configurable parameters that can be set using
...
The custom resource definitions are auto generated by the Verrazzano project `verrazzano-crd-generator`.
The steps to include changes made to the YAML files that create the custom resource definitions are:
