...
func checkFluentdEnabled(t *testing.T, helidonApp *v1helidonapp.HelidonApp, appName string) {
	...
	assert.Equal(t, util.GetFluentdKubernetesDaemonsetImage(), helidonApp.Spec.Containers[0].Image, fmt.Sprintf("Expect image name to be %s", util.GetFluentdKubernetesDaemonsetImage()))
