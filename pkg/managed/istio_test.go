...
import (
	...
	"github.com/verrazzano/verrazzano-operator/pkg/testutils"
...
func TestGetUniqueServiceEntryAddress(t *testing.T) {
	clusterConnections := testutils.GetManagedClusterConnections()
...
func TestGetIstioGateways(t *testing.T) {
	modelBindingPair := testutils.GetModelBindingPair()
	clusterConnections := testutils.GetManagedClusterConnections()
	...
	if err != nil {
		t.Fatal(fmt.Sprintf("can't get istio-ingressgateway service: %v", err))
	}
	...
	if err != nil {
		t.Fatal(fmt.Sprintf("can't update istio-ingressgateway service: %v", err))
	}
...
func TestCreateServiceEntries(t *testing.T) {
	...
	if err != nil {
		t.Fatal(fmt.Sprintf("can't update service entry: %v", err))
	}
...
func assertCreateAuthorizationPolicies(t *testing.T, clusterConnection *util.ManagedClusterConnection, wlsHasConn bool) {
	...
	assert.True(t, testutils.StringArrayContains(policy.Spec.Rules[0].From[0].Source.Namespaces, "test"))
	assert.True(t, testutils.StringArrayContains(policy.Spec.Rules[0].From[0].Source.Namespaces, "istio-system"))
	...
	if wlsHasConn {
		...
		assert.True(t, testutils.StringArrayContains(policy.Spec.Rules[0].From[0].Source.Namespaces, "test2"))
		assert.True(t, testutils.StringArrayContains(policy.Spec.Rules[0].From[0].Source.Namespaces, "test3"))
		assert.True(t, testutils.StringArrayContains(policy.Spec.Rules[0].From[0].Source.Namespaces, "istio-system"))
	} else {
		assert.Equal(t, 2, len(policy.Spec.Rules[0].From[0].Source.Namespaces))
		assert.True(t, testutils.StringArrayContains(policy.Spec.Rules[0].From[0].Source.Namespaces, "test2"))
