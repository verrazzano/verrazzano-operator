...
func createUpdateConfigMaps(managedClusterConnection *util.ManagedClusterConnection, newConfigMap *corev1.ConfigMap, clusterName string) error {
	...
	if existingcm != nil {
		...
		if specDiffs != "" {
			...
			logger.Info().Msgf("Updating ConfigMap %s:%s in cluster %s", newConfigMap.Namespace, newConfigMap.Name, clusterName)
		...
		logger.Info().Msgf("Creating ConfigMap %s:%s in cluster %s", newConfigMap.Namespace, newConfigMap.Name, clusterName)
...
func CleanupOrphanedConfigMaps(mbPair *types.ModelBindingPair, availableManagedClusterConnections map[string]*util.ManagedClusterConnection) error {
	...
	for clusterName, managedClusterConnection := range unmatchedClusters {
		...
		for _, configMap := range existingConfigMapsList {
			logger.Info().Msgf("Deleting ConfigMap %s:%s in cluster %s", configMap.Namespace, configMap.Name, clusterName)
