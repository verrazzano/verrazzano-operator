...
func TestNewDeployment(t *testing.T) {
	...
	assert.Equal("-c", fluentd.Args[0], "Fluent container arg not equal to expected value")
	assert.Equal("/etc/fluent.conf", fluentd.Args[1], "Fluent container arg not equal to expected value")
	assert.Equal(7, len(fluentd.Env), "Fluent container envs arg count not equal to expected value")
	assert.Equal("APPLICATION_NAME", fluentd.Env[0].Name, "Fluent container envs arg name not equal to expected value")
	assert.Equal("test-generic", fluentd.Env[0].Value, "Fluent container envs arg value not equal to expected value")
	assert.Equal("FLUENTD_CONF", fluentd.Env[1].Name, "Fluent container envs arg name not equal to expected value")
	assert.Equal("fluentd.conf", fluentd.Env[1].Value, "Fluent container envs arg value not equal to expected value")
	assert.Equal("FLUENT_ELASTICSEARCH_SED_DISABLE", fluentd.Env[2].Name, "Fluent container envs arg name not equal to expected value")
	assert.Equal("true", fluentd.Env[2].Value, "Fluent container envs arg value not equal to expected value")
	assert.Equal("ELASTICSEARCH_HOST", fluentd.Env[3].Name, "Fluent container envs arg name not equal to expected value")
	assert.Equal("vmi-test-binding-es-ingest.verrazzano-system.svc.cluster.local", fluentd.Env[3].Value, "Fluent container envs arg value not equal to expected value")
	assert.Equal("ELASTICSEARCH_PORT", fluentd.Env[4].Name, "Fluent container envs arg name not equal to expected value")
	assert.Equal("9200", fluentd.Env[4].Value, "Fluent container envs arg value not equal to expected value")
	assert.Equal("ELASTICSEARCH_USER", fluentd.Env[5].Name, "Fluent container envs arg name not equal to expected value")
	assert.Equal(constants.VmiSecretName, fluentd.Env[5].ValueFrom.SecretKeyRef.Name, "Fluent container envs arg value from secret name not equal to expected value")
	assert.Equal("username", fluentd.Env[5].ValueFrom.SecretKeyRef.Key, "Fluent container envs arg value from secret key not equal to expected value")
	assert.Equal(true, *fluentd.Env[5].ValueFrom.SecretKeyRef.Optional, "Fluent container envs arg value from secret optional not equal to expected value")
	assert.Equal(constants.VmiSecretName, fluentd.Env[5].ValueFrom.SecretKeyRef.Name, "Fluent container envs arg value from secret name not equal to expected value")
	assert.Equal("password", fluentd.Env[6].ValueFrom.SecretKeyRef.Key, "Fluent container envs arg value from secret key not equal to expected value")
	assert.Equal(true, *fluentd.Env[6].ValueFrom.SecretKeyRef.Optional, "Fluent container envs arg value from secret optional not equal to expected value")
	assert.Equal(3, len(fluentd.VolumeMounts), "Fluent container volume mounts arg count not equal to expected value")
	assert.Equal("/fluentd/etc/fluentd.conf", fluentd.VolumeMounts[0].MountPath, "Fluent container volume mounts mount path not equal to expected value")
	assert.Equal("fluentd-config-volume", fluentd.VolumeMounts[0].Name, "Fluent container volume mounts name not equal to expected value")
	assert.Equal("fluentd.conf", fluentd.VolumeMounts[0].SubPath, "Fluent container volume mounts sub path not equal to expected value")
	assert.Equal(true, fluentd.VolumeMounts[0].ReadOnly, "Fluent container volume mounts mount read only flag not equal to expected value")
	assert.Equal("/var/log", fluentd.VolumeMounts[1].MountPath, "Fluent container volume mounts mount path not equal to expected value")
	assert.Equal("varlog", fluentd.VolumeMounts[1].Name, "Fluent container volume mounts name not equal to expected value")
	assert.Equal(true, fluentd.VolumeMounts[1].ReadOnly, "Fluent container volume mounts mount read only flag not equal to expected value")
	assert.Equal("/u01/data/docker/containers", fluentd.VolumeMounts[2].MountPath, "Fluent container volume mounts mount path not equal to expected value")
	assert.Equal("datadockercontainers", fluentd.VolumeMounts[2].Name, "Fluent container volume mounts name not equal to expected value")
	assert.Equal(true, fluentd.VolumeMounts[2].ReadOnly, "Fluent container volume mounts mount read only flag not equal to expected value")

	// Check for volumes created for Fluent
	volumes := deploy.Spec.Template.Spec.Volumes
	assert.Equal(3, len(volumes), "Fluent container volumes arg count not equal to expected value")
	assert.Equal("varlog", volumes[0].Name, "Fluent volume name not equal to expected value")
	assert.Equal("/var/log", volumes[0].VolumeSource.HostPath.Path, "Fluent volume host path not equal to expected value")
	assert.Equal("datadockercontainers", volumes[1].Name, "Fluent volume name not equal to expected value")
	assert.Equal("/u01/data/docker/containers", volumes[1].VolumeSource.HostPath.Path, "Fluent volume host path not equal to expected value")
	assert.Equal("fluentd-config-volume", volumes[2].Name, "Fluent volume name not equal to expected value")
	assert.Equal("test-generic-fluentd", volumes[2].VolumeSource.ConfigMap.Name, "Fluent volume config map name not equal to expected value")
	assert.Equal(int32(420), *volumes[2].VolumeSource.ConfigMap.DefaultMode, "Fluent volume config map default mode not equal to expected value")

	// Unset the replica value to make sure default value of 1 is used.
	generic.Replicas = nil

	// Disable Fluent and make sure fluentd container is not included in deployment.
